...
-- MENU CATEGORY
-- ==========================================
menu:category("Ordnance Drops", function(ba)
-- Ordered from estimated smallest blast to largest
  ba:entry("Gunship Shell",          function() dropOrdnance(ctx, "Gunship Shell", 40, 30, "distance", 40, -90, "IMPACT", 2) end)
  ba:entry("Heavy Artillery",        function() dropOrdnance(ctx, "Artillery Shell", 50, 40, "distance", 50, -90, "IMPACT", 2) end)
  ba:entry("Rocket Artillery",       function() dropOrdnance(ctx, "Rocket Artillery Projectile", 80, 80, "distance", 90, -90, "IMPACT", 2) end)
  ba:entry("Cluster Bomb",           function() dropOrdnance(ctx, "Cluster Bomb Projectile", 70, 90, "distance", 90, -90, "CLUSTER", -10) end)
  ba:entry("Laser Guided Bomb",      function() dropOrdnance(ctx, "Laser Guided Bomb Projectile", 50, 60, "distance", 65, -100, "IMPACT", 2) end)
  ba:entry("Fuel Air Bomb",          function() dropOrdnance(ctx, "Fuel Air Bomb Projectile", 60, 95, "distance", 105, -90, "FAB", 0) end)
  ba:entry("Smart Bomb",             function() dropOrdnance(ctx, "Smart Bomb Projectile", 60, 70, "distance", 75, -95, "IMPACT", 2) end)
  ba:entry("Daisy Cutter",           function() dropOrdnance(ctx, "Daisy Cutter Projectile", 120, 100, "distance", 110, -75, "IMPACT", 0) end)
  ba:entry("Cruise Missile",         function() dropOrdnance(ctx, "Cruise Missile Projectile", 100, 70, "distance", 80, -85, "IMPACT", 2) end)
  ba:entry("MOAB",                   function() dropOrdnance(ctx, "MOAB Projectile", 150, 120, "distance", 130, -65, "IMPACT", 0) end)
  ba:entry("Tactical Nuke (Single)", function() dropNukes(ctx, false) end)
  ba:entry("Tactical Nuke (Ring 8x)", function() dropNukes(ctx, true) end)
end)
